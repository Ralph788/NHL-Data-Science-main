...
def main():
    ...
    X_resampled, y_resample = smote.fit_resample(X_train_scaled, y_train)
    ...
    exp = create_experiment("mlp_hypertuned_KBest_smote")
    log_metrics(exp, y_val, y_pred, y_pred_proba)
    log_plots(exp, y_val.values, y_pred_proba)
    log_model(
        exp,
        model,
        "mlp_hypertuned_KBest_smote",
        ["MLPClassifier", "Features: features with KBest and smote"],
    )
    exp.end()
