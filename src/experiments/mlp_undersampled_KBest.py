...
def main():
    ...
    X_resampled, y_resample = rus.fit_resample(X_train_scaled, y_train)
    ...
    exp = create_experiment("mlp_undersampled_KBest")
    log_metrics(exp, y_val, y_pred, y_pred_proba)
    log_plots(exp, y_val.values, y_pred_proba)
    log_model(
        exp,
        model,
        "mlp_undersampled_KBest",
        ["MLPClassifier", "Features: features with KBest and undersampling"],
    )
    exp.end()
